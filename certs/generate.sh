...
for i in {0..2} ; do
  generate_certificate \
    ...
    "subjectAltName=DNS:broker-${i},DNS:broker-${i}.demodomain,DNS:kafka.siemens.ps.confluent.io,DNS:b-${i}.kafka.siemens.ps.confluent.io\nbasicConstraints=CA:FALSE\nkeyUsage=digitalSignature,keyEncipherment\nextendedKeyUsage=serverAuth" \
    sslca
done
...
for i in {0..1} ; do
  generate_certificate \
    ...
    "subjectAltName=DNS:registry-${i},DNS:registry-${i}.demodomain,DNS:registry.siemens.ps.confluent.io\nbasicConstraints=CA:FALSE\nkeyUsage=digitalSignature,keyEncipherment\nextendedKeyUsage=serverAuth" \
    sslca
done
...
for i in {0..0} ; do
  generate_certificate \
    ...
    "subjectAltName=DNS:c3-${i},DNS:c3-${i}.demodomain,DNS:c3.siemens.ps.confluent.io\nbasicConstraints=CA:FALSE\nkeyUsage=digitalSignature,keyEncipherment\nextendedKeyUsage=serverAuth" \
    sslca
done
...
for i in {0..1} ; do
  generate_certificate \
    ...
    "subjectAltName=DNS:connect-${i},DNS:connect-${i}.demodomain,DNS:connect.siemens.ps.confluent.io\nbasicConstraints=CA:FALSE\nkeyUsage=digitalSignature,keyEncipherment\nextendedKeyUsage=serverAuth" \
    sslca
done
...
for i in {0..1} ; do
  generate_certificate \
    ...
    "subjectAltName=DNS:replicator-${i},DNS:replicator-${i}.demodomain,DNS:replicator.siemens.ps.confluent.io\nbasicConstraints=CA:FALSE\nkeyUsage=digitalSignature,keyEncipherment\nextendedKeyUsage=serverAuth" \
    sslca
done