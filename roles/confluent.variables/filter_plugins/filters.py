class FilterModule(object):
    ...
    def normalize_sasl_protocol(self, protocol):
        ...
        normalized = 'GSSAPI' if protocol.lower() == 'kerberos' \
            ...
            else 'custom' if protocol.lower() == 'custom' \
            else 'none'
        return normalized
    ...
    def kafka_protocol_normalized(self, sasl_protocol_normalized, ssl_enabled):
        ...
        kafka_protocol = 'SASL_SSL' if ssl_enabled == True and sasl_protocol_normalized in ['GSSAPI', 'PLAIN', 'SCRAM-SHA-512', 'SCRAM-SHA-256', 'OAUTHBEARER', 'custom'] \
            else 'SASL_PLAINTEXT' if ssl_enabled == False and sasl_protocol_normalized in ['GSSAPI', 'PLAIN', 'SCRAM-SHA-512', 'SCRAM-SHA-256', 'OAUTHBEARER', 'custom'] \
